...
#         elif has_seen_audio_token and token is None and token_text.strip() and token_text.strip() != '>' and not token_text.startswith('<custom_token_'):
#             print(f"WARNING: Detected non-audio token '{token_text.strip()}' after audio stream. Assuming end of audio.")
#             spurious_text_detected = True
...
async def tokens_decoder(token_gen) -> Generator[bytes, None, None]:
    ...
    async for token_text in token_gen:
        ...
        # --- Handle Spurious Text Detection (Same as before) ---
            ...
            print(f"[Decoder] WARNING: Detected non-audio token '{token_text.strip()}' after audio. Breaking loop.")
            spurious_text_detected = True
            break
